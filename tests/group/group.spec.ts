...
describe('GET /groups', () => {
  it('Should get a group by groupname', async () => {
    const response = await request(app).get('/groups?name=GrupoAday').expect(200);
...
describe('PATCH /groups', () => {
  it('Should update a group by query', async () => {
    const response = await request(app).patch(`/groups?name=GrupoAday`).send({
                      name : "GrupoCambiado",
                    }).expect(200);
    expect(response.body).to.include({
      name: "GrupoCambiado",
...
describe('DELETE /groups', () => {
  it('Should delete a group by query', async () => {
    const response = await request(app).delete(`/groups?name=GrupoAday`).expect(200);
    expect(response.body).to.include({
      name: "GrupoAday",
    });
    const secondGroup = await Group.findById(response.body._id);
